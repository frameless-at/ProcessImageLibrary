...
- **Automatic de-duplication** — byte-identical images are fingerprinted (`content_hash`) and collapsed onto one hardlinked inode (lossless, reversible; originals + variations + page-version files). Runs on save + hourly (`LazyCron`) + an install pass; a *Duplicates* filter, copy-count badges and a cluster expand / modal surface them, and the config page offers Scan / Re-measure / Revert tools (see [De-duplication](#de-duplication))
...
- `___executeScanStep()` / `___executeReclaimStep()` / `___executeRevertStep()` / `___executeDiskAudit()` — chunked, time-budgeted de-dup endpoints driving the config page's live "Scan and reclaim" / "Revert" / "Re-measure" tools (fingerprint scan, hardlink reclaim, un-share, real-disk audit).
