...
- `___executeUsage()` — AJAX POST, where-used preflight for the delete confirm dialog. Accepts `items=[{pageId, basename}, …]`, returns `usage: { "pid:basename": [ {pageId, pageTitle, editUrl, fieldName}, … ] }`. Reverse-scans every `FieldtypeTextarea` with `contentType >= contentTypeHTML` via `LIKE '%/{pid}/{basename}%'` over the `data` column + any multilang `data{langId}` columns (detected with `SHOW COLUMNS`). Editor-agnostic — both `pwimage` plugins (CKEditor + TinyMCE) insert the same URL shape. Pages the current user can't `viewable()` are dropped before the response so titles + edit URLs don't leak across access boundaries.
