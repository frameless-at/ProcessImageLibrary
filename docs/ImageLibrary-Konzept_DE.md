...
- `___executeUsage()` — AJAX-POST, Where-used-Preflight für den Delete-Confirm-Dialog. Akzeptiert `items=[{pageId, basename}, …]`, liefert `usage: { "pid:basename": [ {pageId, pageTitle, editUrl, fieldName}, … ] }`. Reverse-Scan über jedes `FieldtypeTextarea` mit `contentType >= contentTypeHTML` per `LIKE '%/{pid}/{basename}%'` auf der `data`-Spalte + allen Multilang-`data{langId}`-Spalten (via `SHOW COLUMNS` ermittelt). Editor-agnostisch — beide `pwimage`-Plugins (CKEditor + TinyMCE) schreiben dasselbe URL-Schema. Pages die der aktuelle User nicht `viewable()` sieht, werden vor dem Response gedroppt, damit Titel + Edit-URLs keine Access-Boundaries leaken.
