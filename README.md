...
- **Hookable `resolvePageRef()`** – a new hookable method that produces the **“Page” column** reference for each row (page id, title, front-end URL, edit URL, name). The default reproduces current behaviour exactly (repeater/matrix owner page, otherwise the storage page), so with no hook attached nothing changes. Third-party code can hook it to retarget images that live on *infrastructure* pages an editor never opens via `page/edit`:
  - **MediaHub** asset pages (`pkd-mediahub-asset`): keep the id, point the link at the asset screen (`…/setup/media-hub/asset/?id=<id>`) instead of `page/edit`.
  - **RockPageBuilder** block pages (`rockpagebuilderblock-*`): repoint the whole reference to the parent content page.

  Copy-paste hook examples for both are in the `___resolvePageRef()` doc block. Scope is deliberately the **“Page” column only** (link + title): the per-image editor modal still targets the storage page’s real image-field slot, and “Used in” (rich-text embeds) is unaffected. Known limitation: the hook resolves the *displayed* link/title; sorting and filtering still key off the storage page’s cached title (a non-issue for MediaHub, visible for RockPageBuilder only when sorting/filtering by the Page column).
